本篇作为基于项目协作的**基础篇**，主要从协作的视角介绍项目成员如何基于项目进行团队内各项任务的上传下达及信息传递，从而提高团队整体工作效率。
...
表格顾名思义就是以表格的形式展示所有的任务及信息，与列表展示形式类似，不同的是，表格有表头，且表头的展示项可以自定义设置。

![](/assets/项目-表格.png)

##### 3.2 展示任务时间属性——时间、日历

不同的任务在执行过程中，可能会有多个时间节点，很多情况下项目负责人需要清楚的知道哪些任务目前处于什么哪个时间节点，所以，为了满足这一需求，Worktile提供两种展示任务时间属性的组件：时间和日历。

**1）时间组件——甘特图**

时间组件也可以叫做甘特图组件，通过条状图来显示项目或任务随时间变化的情况，是项目管理过程中非常重要的工具。

![](/assets/项目-甘特图.jpeg)

**2）日历组件——任务日历**

日历组件也可以称为任务日历，主要作用是展示任务在某一时间点或时间段分布情况。

![](/assets/项目-日历组件.jpeg)

##### 3.3 工时统计——工时

工时是很多企业或项目估算控制成本的途径之一，记录并统计每个任务从开始执行到最终完成所需的时间可以有效控制项目成本。

![](/assets/项目-工时.png)

> 提示：工时组件汇总的是带有“工时”属性的任务。

![](/assets/项目-工时任务.png)

##### 3.4 规划迭代——迭代

敏捷开发中，迭代的周期以及每个周期要做的任务和bug都需要提前规划，这样不仅能提高研发效率还可以保证及时交付。迭代组件是专为敏捷开发设计的组件，主要用于规划迭代并显示迭代进度。

![](/assets/项目-迭代.png)

##### 3.5 文档管理-文件

同一项目中，不同任务会有不同的文档资料，甚至同一任务中会有多个文件，当任务数量较多或任务时间较长的情况下，想找到某个任务中的某一文件需要逐条查看任务中的附件，为了解决这一问题，Worktile提供文件组件，用于汇总和管理项目中的所有文档资料，方便随时查阅。

![](/assets/项目-文件管理.png)

#### 4. 我的工作台

工作台提供的是项目之外的视角，有6个工作组件，分别提供不同视角的任务情况。  
【**我的任务**】展示所有与“我”有关的任务，包括：负责人是“我”、分配人是“我”、参与人中包括“我”、创建人是“我”的任务；  
【**统计报表**】从项目和成员角度分别统计任务的进度和延期情况；  
【**工时统计**】提供全局视角的工时明显、汇总并从不同维度对工时进行统计；  
【**时间视图**】显示所属所有项目下的任务进度情况；  
【**人员视图**】统计所在部门所有成员的任务（包括平级及下级）；  
【**日历视图**】展示个人及团队的任务在某一时间点或时间段分布情况（所展示任务的时间属性可自定义设置）；

![](/assets/项目-工作台.gif)

> 提示：所有工作组件的名称均可在【配置中心】进行修改，还可对分组等进行修改和配置。
