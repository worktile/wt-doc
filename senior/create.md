...
配置前，先[了解项目模板中的元部件及其功能](#1-了解项目模板中的元部件及其功能)；
然后，再[开始配置自己的项目模板](#2-配置我的项目模板)。
...
可以看到新建的任务类型包含状态、属性、工作流、角色、关联等配置项，具体每个配置项的意义及作用，我们在之前的[任务类型](#12-任务类型)中已经介绍过了，这里我们直接针对“内部征稿”这一任务类型进行具体配置；
...
将之前已经[配置好的任务类型](#21-任务类型配置)“添加”到当前项目模板中：
...
之前关于[每个组件的功能及作用](#11-项目组件)已经做了详细介绍，所以针对内容营销这个项目中所需的组件主要有四点要求：
（1）展示所有稿件及其发布状态；
（2）一个发稿日历展示每天的发稿情况；
