...
**2）权限模式**
我们知道不同角色在项目中拥有的权限不同，这里的权限不仅是对整个项目的操作权限（角色模式），更是对任务的操作权限，所以我们用权限模式来规定不同角色模式下**角色在任务中的操作权限**。具体配置步骤如下：
...
**3）安全模式**
角色和权限模式分别决定不同角色对项目、任务的操作权限，而安全模式则是决定不同角色对任务的可见性。所以，安全模式也是基于角色的基础上
