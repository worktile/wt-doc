...
【应用管理】-【简报】主要用于简报应用可见性及简报模板的管理。基本设置主要设置简报应用的可见性，默认全员可见，这里不再多做介绍；关于简报模板的管理及设置之前在[【如何使用】-【工作汇报】](/how/report.md)中已经做了详细的说明和介绍，这里不再介绍。
...
目标应用的设置也包括两部分，基本设置是对目标可见范围的设置，默认全员可见，企业可根据需要设置可见性，这里不多做说明；重点介绍一下目标管理：

![](/assets/应用管理-目标管理.png)

通过目标应用管理后台，可对公司级目标数量、单一部门部门级目标数量、单一成员负责的目标数量及单一目标的关键结果数量上限进行限制，还可以设置父子目标之间的差值百分比（设定百分比后，如果父子目标的差值超过设定值，父目标或子目标就会出现在差值分析中）。

##### 2.8 公告应用设置

公告应用的后台管理仅是对可见范围的管理，这里不再赘述。
